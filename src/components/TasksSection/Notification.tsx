...
const Notification: React.FC = () => {
  ...
  const { tasks: incompletedTasks } = useCompletedTasks({
    tasks: todaysTasks,
    done: false,
  });

  const tasksToShow = incompletedTasks.slice(0, 3);

  const moreTasksToShow = incompletedTasks.length > tasksToShow.length;
  return (
          ...
          {incompletedTasks.length > 0 ? (
            <div>
              <span className="dark:text-slate-200 font-medium">
                You have {incompletedTasks.length} uncompleted tasks today:
              </span>
              <ul>
